...
$pinq = $mysqli->query("select * from farm_watering ORDER BY id DESC");
?>
<style>
    .dataTables_filter {
        display: none;
                ...
                while ($fetch = $pinq->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <td>
                            <button type="button"
                                    data-type="confirm"
                                    class="btn btn-sm btn-primary edit_watering"
                            ...
                            <button type="button"
                                    data-type="confirm"
                                    class="btn btn-sm btn-danger delete_watering"
                                    i_index="<?php echo $fetch['id']; ?>"
                                    title="Delete">
...
<script>
    ...
    $(document).off('click', '.delete_watering').on('click', '.delete_watering', function () {
        var theindex = $(this).attr('i_index');
        //alert(theindex)
        $.confirm({
            title: 'Delete Watering Activity!',
            ...
            buttons: {
                ...
                yes: {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "ajax/queries/delete_farmwatering.php",
                            data: {
                                i_index: theindex
                            },
                            dataType: "html",
    ...
    $(document).off('click', '.edit_watering').on('click', '.edit_watering', function () {
        var theindex = $(this).attr('i_index');
        //alert(theindex)
        $.ajax({
            type: "POST",
            url: "ajax/forms/farmwatering_formedit.php",
            beforeSend: function () {
                KTApp.blockPage({
                    overlayColor: "#000000",
                    type: "v2",

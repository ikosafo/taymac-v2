...
$pinq = $mysqli->query("select * from taymac_image_install ORDER BY id DESC");
?>
<style>
    .dataTables_filter {
        display: none;
        ...
        <div class="table-responsive">
            <table id="data-table" class="table" style="margin-top: 3% !important;">
                ...
                while ($fetch = $pinq->fetch_assoc()) {
                    ?>
                    <tr>
                        <td><img src="<?php echo $fetch['image_location'] ?>" width="150" height="100"/></td>
                        <td><?php echo $fetch['dateuploaded']; ?></td>
                        <td>
                            <button type="button"
                                    data-type="confirm"
                                    class="btn btn-danger delete_install"
...
<script>
    ...
    $(document).off('click', '.delete_install').on('click', '.delete_install', function () {
            ...
            buttons: {
                ...
                yes: {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "ajax/queries/delete_install.php",
                            data: {
                                i_index: theindex
                            },
                            dataType: "html",

...
class tableDataObject{
    ...
    public static function RecordCount(){
        ...
        global $healthdb;
        $healthdb->prepare("select count(*) as recordcount from $tablename");
        $getcount = $healthdb->fetchColumn();
        return $getcount;
    }
	...
	public static function listAll($createObjects = false, $clausearray = null){
      ...
      $clausequery =  $orderby. ' '. $limit. ' '. $offset ;
        ...
        global $healthdb;
        if($clausearray == null){
        $healthdb->prepare("select * from $tablename order by $tableinfo->primaryKey");
        }else{
           $healthdb->prepare("select * from $tablename $clausequery");
        }

        if (!$createObjects){
        	$results = $healthdb->resultSet();
        } else {
        	foreach ($healthdb->resultSet() as $resultrecord){
    ...
    public function store($forcenulls = false){
        ...
        if(!isset($this->recordObject->$pk)){
            ...
            $healthdb->prepare($pqry);

            AppLogs::dblog("inserting into $this->tableName with query: $pqry");
            ...
            AppLogs::dblog("updating $this->tableName with query: $pqry");
        }
        /*
        ...
        for ($fcount = 0; $fcount < count($fields); $fcount++) {
            if (isset($values[$fcount])) {
                $healthdb->bind($binders[$fcount], $values[$fcount]);
            }
        }
        if ($healthdb->execute()){
            if($mode == 'insert'){
                $this->recordObject->$pk = $healthdb->lastInsertId();
            }
            return $this;
        } else {
            /*
            ...
            AppLogs::dblog("error in inserting or updating");
        }
